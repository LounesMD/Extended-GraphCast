...
from utils import plot_mesh_density_map, plot_mesh_statistics, visualize_graphcast_meshes, visualize_graphcast_meshes_direct, visualize_grid_mesh_comparison, visualize_input_grid, visualize_mesh_hierarchy_connections, visualize_mesh_levels_separately, visualize_regional_mesh_details
...
class GraphCast(predictor_base.Predictor):
  ...
  * Mesh2Grid graph: Graph that contains all nodes. This graph is strictly
  ...
  def __init__(self, model_config: ModelConfig, task_config: TaskConfig):
    ...
    self._meshes = (
        icosahedral_mesh.get_hierarchy_of_triangular_meshes_for_sphere(
            splits = 0 # model_config.mesh_size
            ))
    # self._meshes = icosahedral_mesh.get_regional_hierarchy_with_8way_split(
    #     splits = 2,
    #     center_lat = 48.0,
    #     center_lon = -2.75,
    #     regional_radius_deg = 30)
  ...
  def __call__(self,
    ...
    res = self._grid_node_outputs_to_prediction(output_grid_nodes, targets_template)
    
    # # The grid coordinates are stored in self._grid_lat and self._grid_lon
    # fig1 = visualize_input_grid(self._grid_lat, self._grid_lon)
    # plt.show()

    # # Compare grid and mesh
    # fig2 = visualize_grid_mesh_comparison(self._grid_lat, self._grid_lon, self._meshes)
    # plt.show()

    # # Visualize each level separately
    # fig1 = visualize_mesh_levels_separately(self._meshes)
    # plt.show()

    # # Show refinement pattern
    # fig2 = visualize_mesh_hierarchy_connections(self._meshes)
    # plt.show()

    # fig = visualize_regional_mesh_details(
    #     self._meshes, 
    #     # center_lat=50.0,  # Your region center
    #     # center_lon=10.0,
    #     # zoom_radius_deg=40  # How wide to show in regional view
    # )
    # plt.show()

    # # # Also create density heatmap for specific level
    # # fig_density = plot_mesh_density_map(
    # #     self._meshes[2],  # Level 2
    # #     center_lat=50.0,
    # #     center_lon=10.0,
    # #     level=2
    # # )
